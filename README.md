...
[licença CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). O MVP é
educacional; requisitos de atribuição, volume e licença devem ser reconferidos
...
O primeiro comando executa a regressão completa, atualmente com 250 testes e
sem acesso real à internet. Os dois últimos validam especificamente a
documentação OpenAPI e a conformidade entre contrato, Swagger, schemas,
arquivos JSON e links Markdown. Recarregue a `DATABASE_URL` do `.env` antes de
iniciar novamente a API contra o MySQL.
